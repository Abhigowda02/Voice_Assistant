...
def getWeather(city_name):
    ...
    if x["cod"] != "404":
        ...
        print(info)
        speak(f"Here is the weather report at {city_name}")
        speak(info)
    else:
...
def callVoiceAssistant():
    ...
    while True:
        ...
        if "jit 1 point o" in command:
            ...
            curTime = f"{strTime.hour} hours {strTime.minute} minutes {strTime.second} seconds"
            speak(uname)
            ...
            # You need to implement this part since `ec.capture` is not defined
            pass
...
Label(wn, text='Welcome to meet the Voice Assistant by JIT', bg='darkblue',
...
cmdLabel = Label(wn, textvariable=showCommand, bg='darkblue',
